...
namespace CCSheet.Menus
{
	...
	{
		internal static string CSText(string key, string category = "RecipeBrowser") => CCSheet.CSText(category, key);
		...
		public RecipeBrowserWindow(CCSheet mod) {
			categories.Clear();
		...
		public override void Update() {
			...
			if (selectedRecipeChanged) {
				//ErrorLogger.Log("1");
				//foreach(var a in CCSheet.ButtonClicked)
				//{
				//	Main.NewText(">");
