...
namespace CCSheet.Menus
{
	...
	{
		internal static string CSText(string key, string category = "Hotbar") => CCSheet.CSText(category, key);
		...
		public Hotbar(CCSheet mod) {
			...
			this.bCycleExtraAccessorySlots.onLeftClick += (s, e) => {
				CCSheetPlayer cheatSheetPlayer = Main.LocalPlayer.GetModPlayer<CCSheetPlayer>();
				cheatSheetPlayer.numberExtraAccessoriesEnabled = (cheatSheetPlayer.numberExtraAccessoriesEnabled + 1) % (CCSheetPlayer.MaxExtraAccessories + 1);
			...
			this.bCycleExtraAccessorySlots.onRightClick += (s, e) => {
				CCSheetPlayer cheatSheetPlayer = Main.LocalPlayer.GetModPlayer<CCSheetPlayer>();
				cheatSheetPlayer.numberExtraAccessoriesEnabled = cheatSheetPlayer.numberExtraAccessoriesEnabled == 0 ? 0 : (cheatSheetPlayer.numberExtraAccessoriesEnabled - 1) % (CCSheetPlayer.MaxExtraAccessories + 1);
				bCycleExtraAccessorySlots.Tooltip = CSText("ExtraAccessorySlots") + ": " + cheatSheetPlayer.numberExtraAccessoriesEnabled;
			};
		...
		public override void Update() {
			try {
				...
				CCSheet.instance.Logger.Error(e.ToString());
			}
		}
		...
		public void ChangedConfiguration() {
			...
			bToggleRecipeBrowser.Visible = ConfigurationLoader.personalConfiguration.RecipeBrowser && CCSheet.instance.herosPermissions[CCSheet.RecipeBrowser_Permission];
			MinionSlotBooster.button.Visible = ConfigurationLoader.personalConfiguration.MinionBooster && CCSheet.instance.herosPermissions[CCSheet.MinionBooster_Permission];
			bToggleClearMenu.Visible = ConfigurationLoader.personalConfiguration.ClearMenu && CCSheet.instance.herosPermissions[CCSheet.ClearItemNPCProjectile_Permission];
			bTogglePaintTools.Visible = ConfigurationLoader.personalConfiguration.PaintTools && CCSheet.instance.herosPermissions[CCSheet.PaintTools_Permission];
			bToggleExtendedCheat.Visible = ConfigurationLoader.personalConfiguration.ModExtensions && CCSheet.instance.herosPermissions[CCSheet.CCSheetExtensions_Permission];
			bCycleExtraAccessorySlots.Visible = ConfigurationLoader.personalConfiguration.ExtraAccessorySlots && CCSheet.instance.herosPermissions[CCSheet.ExtraAccessories_Permission];
...
//namespace CCSheet.UI
//{
//	class CheatMenu : UIWindow
//	{
//		public Mod mod;
//		private static UIImage[] buttons = new UIImage[CCSheet.ButtonTexture.Count];
...
//			for (int j = 0; j < CCSheet.ButtonTexture.Count; j++)
//			{
//				UIImage button = new UIImage(CCSheet.ButtonTexture[j]);
//				Vector2 position = new Vector2(this.spacing, this.spacing);
//				button.Scale = 32f / Math.Max(CCSheet.ButtonTexture[j].Width, CCSheet.ButtonTexture[j].Height);

//				position.X += (float)(j % 1 * 40);
//				position.Y += (float)(j / 1 * 40);

//				if (CCSheet.ButtonTexture[j].Height > CCSheet.ButtonTexture[j].Width)
//				{
//					position.X += (32 - CCSheet.ButtonTexture[j].Width) / 2;
//				}
//				else if (CCSheet.ButtonTexture[j].Height < CCSheet.ButtonTexture[j].Width)
//				{
//					position.Y += (32 - CCSheet.ButtonTexture[j].Height) / 2;
//				}
...
//			uIImage.Tooltip = CCSheet.ButtonTooltip[num]();
//		}
//	}
//}