...
namespace CCSheet
{
	...
	internal static class PaintToolsEx
	{
		...
		static readonly JsonSerializerOptions TileJsonOpts = new() {
			WriteIndented = true,
			NumberHandling = JsonNumberHandling.AllowNamedFloatingPointLiterals,
		};

		internal static Queue<JsonObject> schematicsToLoad;
		...
		internal static void GetSchematicsComplete(object sender, UploadValuesCompletedEventArgs e) {
			if (!e.Cancelled) {
				...
				try {
					jsonObject = JsonNode.Parse(response)!.AsObject();
				}
				catch (Exception ex) {
				...
				JsonArray schematicslist = jsonObject["schematics"] as JsonArray;
				if (schematicslist != null) {
					schematicsToLoad = new Queue<JsonObject>();
					CCSheet.instance.numberOnlineToLoad = CCSheet.DefaultNumberOnlineToLoad;
					foreach (var node in schematicslist) {
						if (node is JsonObject schematic)
					...
					//if (list.Count > 0)
					//	CCSheet.instance.paintToolsUI.view.Add(list.ToArray());
				}
			}
			else {
		...
		internal static void LoadSingleSchematic() {
			...
			int id = schematic["id"]!.GetValue<int>();
			string name = (string?)schematic["name"];
			int rating = schematic["rating"]!.GetValue<int>();
			int vote = schematic["vote"]!.GetValue<int>();
			string tiledata = (string?)schematic["tiledata"];
			try {
				...
				if (tiles.GetLength(0) > 0) {
					...
					paintToolsSlot.rating = rating;
					paintToolsSlot.vote = vote;
					CCSheet.instance.paintToolsUI.view.AddEndDontSelect(paintToolsSlot);
				}
			}
			catch { }

			CCSheet.instance.numberOnlineToLoad--;
		...
		internal static void OnlineImport(PaintToolsView paintToolsView) {
			...
			if (CCSheet.instance.paintToolsUI.view.slotList.Any(x => x.browserID > 0)) {
		...
		internal static void LoadNextX(PaintToolsView view) {
			...
			CCSheet.instance.paintToolsUI.view.RemoveAllOnline();
			CCSheet.instance.numberOnlineToLoad = CCSheet.DefaultNumberOnlineToLoad;
		}
		...
		internal static void Import(PaintToolsView paintToolsView) {
			try {
				...
				foreach (var line in File.ReadAllLines(importPath, Encoding.UTF8)) {
					TileData[,] tiles = JsonSerializer.Deserialize<TileData[,]>(File.ReadAllText(line, Encoding.UTF8), TileJsonOpts);
		...
		public static void Export(PaintToolsView paintToolsView) {
			try {
				...
				foreach (var slot in paintToolsView.slotList) {
					...
					list.Add(path);
					File.WriteAllText(path, JsonSerializer.Serialize(slot.stampInfo.Tiles, TileJsonOpts), Encoding.UTF8);
				}
				if (0 < list.Count)
