...
namespace CCSheet.Menus
{
	...
	{
		internal static string CSText(string key, string category = "PaintTools") => CCSheet.CSText(category, key);
		internal static string CSTextSB(string key, string category = "SchematicsBrowser") => CCSheet.CSText(category, key);
		...
		public PaintToolsUI(CCSheet mod) {
			categories.Clear();
			...
			uIImage = new UIImage(ModUtils.GetItemTexture(ItemID.TrashCan));
			...
			uIImage = new UIImage(ModUtils.GetItemTexture(ItemID.AlphabetStatueI));
			...
			uIImage = new UIImage(ModUtils.GetItemTexture(ItemID.AlphabetStatueE));
			...
			uIImage.Tooltip = $"Load Next {CCSheet.DefaultNumberOnlineToLoad}";
			this.AddChild(uIImage);
			*/
			...
			upVoteButton = new UIImage(CCSheet.instance.Assets.Request<Texture2D>("UI/VoteUp", ReLogic.Content.AssetRequestMode.ImmediateLoad));
			...
			downVoteButton = new UIImage(CCSheet.instance.Assets.Request<Texture2D>("UI/VoteDown", ReLogic.Content.AssetRequestMode.ImmediateLoad));
		...
		private void Submit() {
			...
			try {
				using (WebClient client = new WebClient()) {
					...
					*/
					...
					if (string.IsNullOrEmpty(base64tiles)) {
						...
						{
							{ "version", CCSheet.instance.Version.ToString() },
							{ "steamid64", steamid64 },
							{ "name", submitInput.Text },
							{ "tiledata", base64tiles },
						};
						PaintToolsSlot.CurrentSelect.browserID = -1;
						CCSheet.instance.paintToolsUI.submitPanel.Visible = false;
				...
				Main.NewText(Language.GetTextValue($"Mods.CCSheet.SchematicsBrowser.SchematicsServerProblemX", 5));
				submitWait = false;
			}
		}
			...
			try {
				if (!e.Cancelled) {
					...
					if (jsonObject["message"] != null) {
						...
						Main.NewText(Language.GetTextValue($"Mods.CCSheet.SchematicsBrowser.SchematicsServerProblemX", 12));
					}
				}
				else {
					Main.NewText(Language.GetTextValue($"Mods.CCSheet.SchematicsBrowser.SchematicsServerProblemX", 2));
				}
			}
			catch {
				Main.NewText(Language.GetTextValue($"Mods.CCSheet.SchematicsBrowser.SchematicsServerProblemX", 3));
			}
			submitWait = false;
		}
		...
		private void Vote(bool up) {
			...
			CCSheet.instance.paintToolsUI.infoMessage.Text = PaintToolsSlot.CurrentSelect.browserName + ": " + PaintToolsSlot.CurrentSelect.rating;
			PaintToolsSlot.CurrentSelect.vote = voteInt;
			CCSheet.instance.paintToolsUI.upVoteButton.ForegroundColor = Color.White;
			CCSheet.instance.paintToolsUI.downVoteButton.ForegroundColor = Color.White;
			if (PaintToolsSlot.CurrentSelect.vote == 1)
				CCSheet.instance.paintToolsUI.upVoteButton.ForegroundColor = Color.Gray;
			if (PaintToolsSlot.CurrentSelect.vote == -1)
				CCSheet.instance.paintToolsUI.downVoteButton.ForegroundColor = Color.Gray;
			if (SocialAPI.Mode != SocialMode.Steam) {
				Main.NewText(CSTextSB("OnlineSchematicsOnlyWorksOnSteamVersion"));
				return;
			}
