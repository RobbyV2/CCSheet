...
namespace CCSheet.Menus
{
	...
	{
		internal static string CSText(string key, string category = "ExtendedCheatMenu") => CCSheet.CSText(category, key);
		public CCSheet mod;
		private static UIImage[] buttons = new UIImage[CCSheet.ButtonTexture.Count];
		private float spacing = 16f;

		public ExtendedCheatMenu(CCSheet mod) {
			buttons = new UIImage[CCSheet.ButtonTexture.Count];
			this.mod = mod;
			this.CanMove = true;
			base.Width = spacing * 2;
			base.Height = spacing * 2;

			if (CCSheet.ButtonTexture.Count == 0) {
			...
			if (CCSheet.ButtonTexture.Count > 0) {
				int count = CCSheet.ButtonTexture.Count;

				int cols = (count + 4) / 5;
				int rows = count >= 5 ? 5 : count;

				for (int j = 0; j < CCSheet.ButtonTexture.Count; j++) {
					Asset<Texture2D> textureAsset = CCSheet.ButtonTexture[j];
