...
namespace CCSheet.Menus
{
	internal class ConfigurationTool
	{
		...
		public static CCSheet cheatSheet;

		public static UIImage GetButton(Mod mod) {
			cheatSheet = mod as CCSheet;
	...
	{
		internal static string CSText(string key, string category = "ConfigurationTool") => CCSheet.CSText(category, key);
		public CCSheet mod;
		private float spacing = 16f;

		public ConfigurationWindow(CCSheet mod) {
			this.mod = mod;
			this.CanMove = true;
			base.Width = 280;
			base.Height = 358;
			...
			for (int i = 0; i < labels.Length; i++) {
				...
				cb.SelectedChanged += (a, b) => {
					...
					((CCSheet)mod).hotbar.ChangedConfiguration();
