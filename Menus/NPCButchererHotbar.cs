...
namespace CCSheet.Menus
{
	internal class NPCButchererHotbar : UIHotbar
	{
		...
		internal static string CSText(string key, string category = "Butcherer") => CCSheet.CSText(category, key);
		...
		public NPCButchererHotbar(CCSheet mod) {
			this.mod = mod;
            ...
            */
			...
			for (int i = 0; i < Main.maxNPCs; i++) // Iteration
			{
				...
				if (npc.active && CheckNPC(i)) {
					if (butcherType == 0 && (npc.townNPC || npc.friendly || NPCID.Sets.CountsAsCritter[npc.type]))
						continue;
					else if (butcherType == 2 && !(npc.townNPC || npc.friendly || NPCID.Sets.CountsAsCritter[npc.type]))
						continue;
