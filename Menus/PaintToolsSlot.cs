...
namespace CCSheet.Menus
{
	internal class PaintToolsSlot : UIView
	{
		...
		public void Select() {
			CCSheet.instance.paintToolsUI.infoPanel.Visible = false;
			CCSheet.instance.paintToolsUI.submitPanel.Visible = false;
			CCSheet.instance.paintToolsUI.submitInput.Text = "";
			if (CurrentSelect != null) {
				CurrentSelect.isSelect = false;
			}
			if (CurrentSelect == this) {
				CurrentSelect = null;
				CCSheet.instance.paintToolsHotbar.StampTiles = new TileData[0, 0];
				CCSheet.instance.paintToolsHotbar.stampInfo = null;
			}
			else {
				isSelect = true;
				CurrentSelect = this;
				CCSheet.instance.paintToolsHotbar.StampTiles = stampInfo.Tiles;
				CCSheet.instance.paintToolsHotbar.stampInfo = stampInfo;

				// Update UI;
				if (CurrentSelect.browserID > 0) {
					CCSheet.instance.paintToolsUI.infoPanel.Visible = true;
					CCSheet.instance.paintToolsUI.infoMessage.Text = browserName + ": " + rating;
					...
					//CCSheet.instance.paintToolsUI.submitPanel.Visible = true;
				}
			}
		}
