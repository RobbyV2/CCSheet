...
namespace CCSheet.Menus
{
	internal class PaintToolsHotbar : UIHotbar
	{
		...
		public PaintToolsHotbar(CCSheet mod) {
			this.mod = mod;
		...
		public override void Draw(SpriteBatch spriteBatch) {
			if (Visible) {
				...
				scissorRectangle = CCSheet.GetClippingRectangle(spriteBatch, scissorRectangle);
			...
			try {
				if (Visible && !base.IsMouseInside() && (StampToolActive || EyeDropperActive)) {
					if (!Main.LocalPlayer.mouseInterface) {
						...
						Terraria.GameInput.PlayerInput.LockVanillaMouseScroll("CCSheet/DrawBrush");

						if (Terraria.GameInput.PlayerInput.ScrollWheelDeltaForUI != 0) {
							if (Terraria.GameInput.PlayerInput.ScrollWheelDeltaForUI < 0)
								CCSheet.instance.paintToolsUI.btnSnap.NextIamge();
							else
								CCSheet.instance.paintToolsUI.btnSnap.PrevIamge();
						}
					}
				}
			}
		...
		private void DrawBrush() {
			if (EyeDropperActive) {
				...
				Vector2 vector = Snap.GetSnapPosition(CCSheet.instance.paintToolsUI.SnapType, width, height, constrainToAxis, constrainedX, constrainedY, false);
			...
			if (selected && (EyeDropperActive || StampToolActive)) {
				...
				if (EyeDropperActive) {
					...
					if (justLeftMouseDown) {
						if (startTileX != -1 && startTileY != -1 && lastMouseTileX != -1 && lastMouseTileY != -1) {
							...
							CCSheet.instance.paintToolsUI.AddSlot(PaintToolsEx.GetStampInfo(StampTiles));
				...
				if (StampToolActive) {
					...
					if (leftMouseDown && stampInfo != null) {
						...
						Point point = Snap.GetSnapPosition(CCSheet.instance.paintToolsUI.SnapType, width, height, constrainToAxis, constrainedX, constrainedY, true).ToPoint();
