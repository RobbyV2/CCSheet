...
namespace CCSheet.Menus
{
	...
	{
		internal static string CSText(string key, string category = "SpawnRate") => CCSheet.CSText(category, key);
		...
		internal static void RequestSetSpawnRate(int index) {
			...
			netMessage.Write((byte)CCSheetMessageType.SetSpawnRate);
		...
		internal static void HandleSetSpawnRate(BinaryReader reader, int whoAmI) {
			...
			return;
			var netMessage = CCSheet.instance.GetPacket();
			netMessage.Write((byte)CCSheetMessageType.SpawnRateSet);
