...
namespace CCSheet
{
	internal class CCSheet : Mod
	{
		internal static ModKeybind ToggleCCSheetHotbarHotKey;
		internal static CCSheet instance;
		...
		internal const string CCSheetExtensions_Permission = "CCSheetExtensions";
		internal const string CCSheetExtensions_Display = "Cheat Sheet Extensions";
		...
		public override void Load() {
			...
			if (BuildInfo.tMLVersion < new Version(2026, 2, 3)) {
			...
			ToggleCCSheetHotbarHotKey = KeybindLoader.RegisterKeybind(this, "ToggleCCSheetHotbar", "K");
			...
			herosPermissions[CCSheetExtensions_Permission] = true;
		...
		public override void Unload() {
			...
			instance = null;
			ToggleCCSheetHotbarHotKey = null;
		...
		internal static string CSText(string category, string key) {
			return Language.GetTextValue($"Mods.CCSheet.{category}.{key}");
			//return translations[$"Mods.CCSheet.{category}.{key}"].GetTranslation(Language.ActiveCulture);
		...
		private void LoadTranslations()
		{
			...
			foreach (var translationFile in translationFiles)
			{
				...
				Dictionary<string, Dictionary<string, string>> dictionary = JsonSerializer.Deserialize<Dictionary<string, Dictionary<string, string>>>(translationFileContents);
		...
		*/
		...
		//	CCSheet.instance.hotbar.ChangedConfiguration();
		//}
		...
		public override void PostSetupContent() {
			...
			try {
				...
				Logger.Error("CCSheet->HEROsMod PostSetupContent Error: " + e.StackTrace + e.Message);
			}
		}
		...
		private void SetupHEROsModIntegration(Mod herosMod) {
			...
			var permissions = new List<ValueTuple<string, string>>() {
				...
				(CCSheetExtensions_Permission,CCSheetExtensions_Display),
		...
		public void SetupUI() {
			...
			if (!Main.dedServ) {
				...
				try {
					...
					hotbar.Visible = true;
					if (!ModContent.GetInstance<CCSheetClientConfig>().HotbarShownByDefault)
						hotbar.Hide();
					else
						hotbar.Show();
				}
		...
		public override void HandlePacket(BinaryReader reader, int whoAmI) {
			...
			// Server relay path; unreachable when running side=Client on vanilla server.
			// Restore for tML-server compatibility if that path is added later.
		}
	...
	public static class CCSheetInterface
	{
		public static void RegisterButton(Asset<Texture2D> texture, Action buttonClickedAction, Func<string> tooltip) {
			if (!Main.dedServ) {
				ModContent.GetInstance<CCSheet>().RegisterButton(texture, buttonClickedAction, tooltip);
			}
		}

		public static void RegisterButton(CCSheetButton csb) {
			if (!Main.dedServ) {
				ModContent.GetInstance<CCSheet>().RegisterButton(csb.texture, csb.buttonClickedAction, csb.tooltip);
			}
		}
		...
		public static IEnumerable<Item> GetExtraAccessories(Player player) {
			return player.GetModPlayer<CCSheetPlayer>().ExtraAccessories;
		}
		...
		public static IEnumerable<Item> GetEnabledExtraAccessories(Player player) {
			var cheatSheetPlayer = player.GetModPlayer<CCSheetPlayer>();
		...
		public static Item GetExtraAccessory(Player player, int index) {
			if (index < 0 || index >= CCSheetPlayer.MaxExtraAccessories)
				return null;
			return player.GetModPlayer<CCSheetPlayer>().ExtraAccessories[index];
		}
	}

	public class CCSheetButton
	{
		internal Asset<Texture2D> texture;

		//internal Action buttonClickedAction;
		//internal Func<string> tooltip;
		public CCSheetButton(Asset<Texture2D> texture/*, Action buttonClickedAction, Func<string> tooltip*/) {
	...
	internal enum CCSheetMessageType : byte
	{
		SpawnNPC,
		QuickClear,
		VacuumItems,
	...
	static class CCSheetUtilities
	{
		...
		internal static void ReportException(Exception e) {
			CCSheet.instance.Logger.Error("CCSheet: " + e.Message + e.StackTrace);
			try {
		...
		class ReportData
		{
			...
			public ReportData() {
				...
				modversion = CCSheet.instance.Version.ToString();
				mod = "CCSheet";
