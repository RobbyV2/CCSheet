...
namespace CCSheet
{
	internal class CCSheetPlayer : ModPlayer
	{
		...
		public override void ProcessTriggers(TriggersSet triggersSet) {
			if (CCSheet.ToggleCCSheetHotbarHotKey.JustPressed) {
				// Debug refresh UI elements
				// CCSheet.instance.paintToolsUI = new Menus.PaintToolsUI(CCSheet.instance);
				if (CCSheet.instance.hotbar.hidden) {
					CCSheet.instance.hotbar.Show();
				}
				else {
					CCSheet.instance.hotbar.Hide();
				}
			}
		}
	}
